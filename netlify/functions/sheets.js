 ...
 * -------------------------------------------------
 * Columnas de la hoja "resources":
 * A title | B url | C desc | D type | E status | F tag | G note | H subgroup | I image
 * J en adelante: checkboxes de categorías (TRUE/FALSE)
 *
 * Hoja "meta": celda A2 contiene =NOW() → se usa como fecha de última actualización.
 * Cada vez que el sheet se edita, Google recalcula =NOW() y esa fecha cambia.
 */
...
// Lee la celda A2 de la hoja "meta" que debe contener =NOW()
// Cada vez que el sheet se edita, Google recalcula esa fórmula automáticamente.
// Formato de celda sugerido: "d mmm yyyy, hh:mm" (ej: 6 jul 2026, 14:32)
async function fetchLastModified() {
  try {
    // Pedir el valor formateado (no el raw) para que respete el formato de la celda
    const url = `https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/meta!A2?key=${API_KEY}&valueRenderOption=FORMATTED_VALUE`;
...
function parseResources(rows) {
  ...
  return rows.slice(1)
    .filter(r => r[0] && r[1])
    .map(r => {
      // Columna I (índice 8) = imagen manual (URL, opcional)
      // Columnas J en adelante (índice 9+) = checkboxes de categorías
      const categories = CAT_IDS.filter((_, i) => (r[9 + i] || "").toUpperCase() === "TRUE");
      return {
        title:    r[0] || "",
        url:      r[1] || "",
        desc:     r[2] || "",
